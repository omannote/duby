...
const FUNCTIONS_URL = `${import.meta.env.VITE_SUPABASE_URL ?? ''}/functions/v1/staff-orders`;
...
/**
 * استدعاء وظيفة الطلبات.
 *
 * تُقرأ الجلسة بـ getSession لا refreshSession: الأخيرة تطلق حدث تغيير جلسة
 * فتعيد تركيب الشجرة وتغلق النافذة قبل إرسال الطلب — وهذا كان السبب الجذري
 * لعطل تأكيد التسليم في النظام السابق. قاعدة ESLint تمنعها.
 */
export async function callOrdersApi<T>(path: string, body: unknown): Promise<ApiResult<T>> {
