...
### تذكير المندوب بالتسوية

المندوب يعمل وحده ولا أحد يذكّره بتسليم النقد. مهمة يومية عند نهاية يوم العمل
(بتوقيت مسقط) تدرج في Outbox تذكيرًا لكل مندوب له تسوية `open`:

```
عميلنا العزيز،
لديك نقد غير مسلَّم بقيمة 18.750 ر.ع لتاريخ 2026-09-16.
يرجى تسليمه وتوثيقه من التطبيق.
```

يستخدم القالب `ar_template` نفسه بمتغير واحد — لا حاجة لاعتماد قالب جديد من
مزود واتساب.

يُضبط من `cash.require_daily_close` و`cash.business_day_start`.
