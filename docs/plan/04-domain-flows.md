...
**إثبات تسليم موثَّق**، لا على عدّ حضوري.
...
sequenceDiagram
    ...
    Note over C,S: التسليم الموثَّق
    C->>C: تسليم النقد (إيداع بنكي / مكتب / خزنة)
    C->>S: رفع صورة الإثبات
    C->>D: fn_hand_over_settlement(declared, method, ref, photo)
    ...
    Note over A,D: لاحقًا — عند ورود كشف الحساب
...
| التسليم وتوثيقه | المندوب | هو من يودع أو يسلّم فعلًا |
...
### طرق التسليم وما يلزمها

| الطريقة | الإثبات | المرجع | المطابقة البنكية |
|---|---|---|---|
| `bank_deposit` | **صورة إيصال إلزامية** | رقم الإيصال | نعم |
| `transfer` | **لقطة التحويل إلزامية** | رقم العملية | نعم |
| `office_handover` | صورة اختيارية | اسم المستلم | لا |
| `safe_drop` | صورة اختيارية | رقم الظرف | لا |

الإيداع البنكي هو الأقوى رقابيًا لأن له مصدر تحقق مستقل عنّا وعن المندوب.
يُفضَّل اعتماده كطريقة افتراضية حيثما أمكن.
...
| 4 | الإيداع والتحويل يتطلبان إثباتًا مصوَّرًا | قيد `CHECK` |
...
| 9 | **الحجب من عدم التسليم لا من تأخر الاعتماد** | فحص `state = 'open'` في `fn_record_cash_payment` |
...
الاعتماد عن بُعد يؤكد **الإثبات**، لا **النقد**. مندوب يصوّر إيصال إيداع بمبلغ
أقل مما حصّل سيظهر الفرق فورًا؛ لكن مندوبًا يؤخّر الإيداع ويصوّره لاحقًا لن
يُكتشف من الصورة وحدها.

التخفيفات المدمجة:

- بصمة الصورة تمنع تكرار نفس الإيصال.
- تاريخ التسليم يُقارن بتاريخ العمل — فجوة متكررة تظهر في التقرير.
- المطابقة البنكية تكشف ما لا تكشفه الصورة.
- «تاريخ الفروق» يكشف الأنماط لا الحوادث المفردة.

ويبقى **العدّ الحضوري المفاجئ** ضابطًا دوريًا — إجراء إداري لا ميزة برمجية.
النظام يوثّق ويكشف الأنماط؛ لا يدّعي أنه يغني عن الثقة والمتابعة.

### مسارات الفشل

| السيناريو | السلوك |
|---|---|
| مندوب نسي التسليم | تذكير واتساب آخر اليوم؛ تحصيل الغد يفتح تسوية جديدة والقديمة تبقى `open` |
| تجاوز `cash.max_unhanded_days` | زر «استلمت المبلغ نقدًا» معطّل برسالة صريحة |
| سلّم ووثّق والمدقّق متأخر | **لا حجب**؛ تنبيه على المدقّق بعد `cash.verify_sla_hours` |
| صورة الإثبات غير واضحة | المدقّق يعيدها `disputed` بسبب، ويطلب إعادة رفع |
| محاولة رفع إيصال مستخدم سابقًا | مرفوض ببصمة `sha256` |
| الإيداع البنكي لم يظهر في الكشف | التسوية معتمدة لكن `bank_matched_at` فارغ ← تقرير المطابقة المعلّقة |
...
|---|---|---|
| تسويتي | المندوب | المتوقّع اليوم، التحصيلات، حالة التسليم |
| بانتظار الاعتماد | `operator`+ | تسويات مسلَّمة مرتبة بالأقدم، مع مؤشر تجاوز الـSLA |
| النقد غير المسلَّم | `operator`+ | مبالغ `open` لكل مندوب حسب العمر ومؤشر الحجب |
| المطابقة البنكية المعلّقة | `operator`+ | إيداعات معتمدة بلا مرجع بنكي |
| تاريخ الفروق | `manager`+ | فروق كل مندوب عبر الزمن، متوسطها واتجاهها |
| فجوة التسليم | `manager`+ | متوسط الزمن بين التحصيل والتسليم لكل مندوب |
