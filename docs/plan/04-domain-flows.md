...
sequenceDiagram
    ...
    Note over C,D: آخر اليوم
    C->>D: fn_submit_settlement(declared_amount)
    D-->>C: submitted — لا تُقبل تحصيلات جديدة فيها

    Note over A,D: التدقيق
    A->>A: عدّ النقد المستلم من المندوب
    A->>D: fn_verify_settlement(counted_amount, reason)
    D->>D: فحص: verified_by <> courier_id
    D->>D: variance = counted − expected

    alt الفرق = 0
        D-->>A: verified
    else الفرق ضمن السماح
        D-->>A: verified مع سبب مسجّل
    else الفرق خارج السماح
        D-->>A: disputed — ينتظر manager+
        A->>D: fn_approve_variance(reason)
        D-->>A: verified
    end
```
...
| إعلان التسوية | المندوب | إقرار بما بحوزته |
| العدّ والاعتماد | `operator`+ | رقابة مستقلة عن التنفيذ |
...
| 2 | `expected_amount` محسوب من النظام، لا يُدخل يدويًا | محفّز `trg_settlement_recalc` |
| 3 | `variance` عمود محسوب — لا يُكتب يدويًا | `GENERATED ALWAYS AS` |
| 4 | فرق غير صفري يتطلب سببًا | قيد `CHECK` |
| 5 | فرق خارج السماح يتطلب اعتماد `manager`+ | قيد `CHECK` |
| 6 | التسوية المعتمدة غير قابلة للتعديل | محفّز `trg_settlement_final_lock` |
| 7 | تسوية متأخرة > الحد ← منع تحصيل جديد | فحص في `fn_record_cash_payment` |
| 8 | تسوية `submitted` لا تقبل تحصيلات جديدة | فحص في الدالة — تُفتح تسوية اليوم التالي |
| 9 | إلغاء التحصيل قبل الاعتماد فقط | فحص حالة التسوية في الدالة |

### مسارات الفشل

| السيناريو | السلوك |
|---|---|
| مندوب نسي الإعلان | التسوية تبقى `open`؛ تنبيه عند إغلاق اليوم؛ تحصيل الغد يفتح تسوية جديدة والقديمة تبقى معلّقة |
| مندوب عليه تسويتان متأخرتان | زر «استلمت المبلغ نقدًا» معطّل برسالة صريحة؛ لا يمكنه التحصيل حتى يسوّي |
| عجز متكرر من مندوب واحد | تقرير «تاريخ فروق المندوب» يُظهر النمط؛ قرار إداري |
| `operator` يحاول اعتماد تسويته | مرفوض في قاعدة البيانات، ليس بإخفاء الزر |
| خطأ في مبلغ تحصيل قبل الاعتماد | إلغاء التحصيل بسبب ثم إعادة تسجيله |
| خطأ اكتُشف بعد الاعتماد | قيد تصحيحي في تسوية لاحقة — لا تعديل على المعتمدة |
| مندوب غادر العمل بتسوية مفتوحة | `manager` يعتمدها بعد العدّ، ويوثّق الظرف في `notes` |

### التقارير

| التقرير | الجمهور | المحتوى |
|---|---|---|
| تسوياتي | المندوب | تسويته اليوم: المتوقّع، عدد التحصيلات، حالتها |
| تسويات اليوم | `operator`+ | كل المندوبين، الحالة، المتوقّع، المعلن، المعدود، الفرق |
| النقد المعلّق | `operator`+ | مبالغ غير مسوّاة لكل مندوب حسب العمر |
| تاريخ الفروق | `manager`+ | فروق كل مندوب عبر الزمن، متوسطها واتجاهها |
| المطابقة اليومية | `manager`+ | مجموع النقد المعتمد مقابل مجموع الطلبات المدفوعة نقدًا |
